...
//----------------------------------------------------------------
// maximum of 20 candidates are considered(worst-case)
...
int authenticateAdmin(){
    ...
    scanf("%s",username);
    if((strcmp(username,"venky"))!=0)
        return 0;
    else
    {
        printf("Enter Admin Password(length=5): ");
        int i=0;
        for(i=0;i<5;i++)
        {
            password[i]=getch();
...
void banID(){
    ...
    int input;
    while(1){
        printf("\nEnter Number or press 0 to exit: ");
        scanf("%d",&input);
        if(input==0)
            break;
        ...
        fprintf(fp,"%d\n",input);
        printf("Ban on user with roll as %d Created Successfully\n",input);
        }
         if((input>=1) && (input<=100000)) break;
...
void deleteIllegalVote(char userID[15])
{
    ...
    char filename[20];
    char line[20];
// taking particular candidate' '.txt which contains illegal vote
    int location = extractRollNo(userID);
    if(location>=1) sprintf(filename,"candidate%d.txt",candidateArray[studentVotes[location-1]-49].cid);
...
int getWinner(){
    ...
    for(int i=0;i<numberOfCandidates;i++)
    {
		if(candidateArray[i].votes == maxV) c++;
	}
	if(c>=2) return -1;
    return winnerCid;
}
        ...
        while(1)
        {
            ...
            switch(input)
            {
                ...
                case '5':
                    ...
                    if(totalVotedNow==0) printf("voted ones are 0");
